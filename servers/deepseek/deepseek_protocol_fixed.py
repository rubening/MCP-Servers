#!/usr/bin/env python3
"""
DeepSeek MCP Server - Protocol Fixed Implementation  
Uses OpenRouter API to access DeepSeek models
Fixed JSON-RPC protocol issues based on working MCP servers
"""
...
async def main():
    ...
    # Check API keys
    if not server.openrouter_api_key and not server.deepseek_api_key:
        logger.error("No API key found. Set OPENROUTER_API_KEY or DEEPSEEK_API_KEY")
        return
    
    provider = "OpenRouter" if server.openrouter_api_key else "DeepSeek Direct"
    logger.info(f"DeepSeek MCP Server starting with {provider} API...")
